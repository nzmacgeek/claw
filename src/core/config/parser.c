...
int config_parse_service(const char *path, service_t *svc) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        else if (strcmp(key, "aliases") == 0 || strcmp(key, "alias") == 0)
            tokenize_into_vector(val, svc->aliases);
...
void config_free(config_t *cfg) {
    ...
    /* Free all services — skip alias entries (key != svc->name) to avoid double-free */
    size_t count = 0;
    char **keys = hashmap_keys(cfg->services, &count);
    for (size_t i = 0; i < count; i++) {
        service_t *svc = hashmap_get(cfg->services, keys[i]);
        if (svc && strcmp(keys[i], svc->name) == 0)
            service_free(svc);
    }
    free(keys);
...
static void load_service_file(const char *path, config_t *cfg) {
    ...
    if (config_parse_service(path, svc) == 0) {
        ...
        for (size_t i = 0; i < vector_length(svc->aliases); i++) {
            const char *alias = vector_get(svc->aliases, i);
            hashmap_set(cfg->services, alias, svc);
            log_debug("config", "  alias: %s -> %s", alias, svc->name);
        }
    } else {
        log_warning("config", "Failed to load service file: %s", path);
        service_free(svc);
    }
}
...
static void load_target_file(const char *path, config_t *cfg) {
    ...
    if (config_parse_target(path, tgt) == 0) {
        ...
        log_info("config", "Loaded target: %s", tgt->name);
    } else {
        log_warning("config", "Failed to load target file: %s", path);
        target_free(tgt);
    }
}

/*
 * Load unit files listed in a manifest.
 *
 * The manifest is a plain-text file generated by `make install` containing
 * one absolute path per line.  Lines beginning with '#' are comments.
 * This approach avoids directory-fd operations (opendir/getdents) that
 * bkernel's VFS does not support.
 *
 * Returns the number of paths loaded, or -1 if the manifest cannot be opened.
 */
static int load_from_manifest(const char *manifest_path, config_t *cfg) {
    FILE *f = fopen(manifest_path, "r");
    if (!f) return -1;

    log_info("config", "Loading units from manifest: %s", manifest_path);
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        /* Determine type by directory component */
        if (strstr(p, "/services.d/")) {
            load_service_file(p, cfg);
        } else if (strstr(p, "/targets.d/")) {
            load_target_file(p, cfg);
        } else {
            log_warning("config", "Manifest entry has unknown type: %s", p);
        }
        count++;
    }
...
/* Scan a single directory for .yml/.yaml files — fallback when no manifest */
static void load_yml_dir(const char *dir,
    ...
    if (!d) {
        int e = errno;
        log_warning("config", "Cannot scan directory: %s (errno %d: %s)",
                    dir, e, strerror(e));
        return;
    }

    log_info("config", "Scanning directory: %s", dir);
    struct dirent *entry;
...
int config_load(config_t *cfg, const char *dir) {
    ...
    /*
     * Prefer the pre-generated manifest (works on kernels that don't support
     * directory fds).  Fall back to directory scanning on normal systems.
     */
    snprintf(path, sizeof(path), "%s/units.manifest", dir);
    if (load_from_manifest(path, cfg) < 0) {
        log_info("config", "No manifest found, falling back to directory scan");
        char svc_dir[512], tgt_dir[512];
        snprintf(svc_dir, sizeof(svc_dir), "%s/services.d", dir);
        snprintf(tgt_dir, sizeof(tgt_dir), "%s/targets.d", dir);
        load_yml_dir(svc_dir, load_service_file, cfg);
        load_yml_dir(tgt_dir, load_target_file, cfg);
    }
